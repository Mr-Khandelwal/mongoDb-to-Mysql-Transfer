        ...
        if isinstance(value, dict):
            ...
            if is_child_key_present_in_value(child_key, value_values):
                ...
                for inner_key in value_keys:
                    new_key = f"{prefix}_{camel_to_snake(inner_key)}" if prefix else camel_to_snake(
                        inner_key)
                    inner_key_value = value[inner_key]
                    max_length = len(str(inner_key_value))
                    structure[new_key] = type_to_mysql(
                        new_key, type(inner_key_value).__name__, max_length)
                    if isinstance(inner_key_value, dict):
                        structure.update(process_nested_document(
                            inner_key_value, new_key))
            else:
                new_key = f"{prefix}_{camel_to_snake(child_key)}" if prefix else camel_to_snake(
                    child_key)
            ...
            new_key = f"{prefix}_{camel_to_snake(child_key)}" if prefix else camel_to_snake(
                child_key)
            structure[new_key] = type_to_mysql(
                new_key, type(value).__name__, max_length)
    return structure
        ...
        if isinstance(doc_value, dict):
            ...
            if is_child_key_present_in_value(child_key, value_values):
                ...
                for inner_key in value_keys:
                    ...
                    if isinstance(inner_key_value, dict):
                        new_document.update(
                            convert_nested_document(inner_key_value, new_key))
            else:
                new_key = f"{prefix}_{camel_to_snake(child_key)}" if prefix else camel_to_snake(
                    child_key)
                new_document.update(
                    convert_nested_document(doc_value, new_key))
        else:
            new_key = f"{prefix}_{camel_to_snake(child_key)}" if prefix else camel_to_snake(
                child_key)
...
def create_mysql_table(mysql_cursor, collection_name, document):
    if document is None:
        print(f"No document found for collection '{collection_name}'")
        return

    collection_name = camel_to_snake(collection_name)
    max_lengths = {camel_to_snake(key): len(
        str(value)) for key, value in document.items() if key not in ['_id', '_class']}
    structure = {}
    for key, value in document.items():
        if key not in ['_id', '_class'] or (key == '_id' and not SKIP_ID_FIELD) or (key == '_class' and not SKIP_CLASS_FIELD):
            if isinstance(value, dict):
                structure.update(process_nested_document(
                    value, camel_to_snake(key)))
            else:
                structure[camel_to_snake(key)] = type_to_mysql(camel_to_snake(
                    key), type(value).__name__, max_lengths.get(camel_to_snake(key)))

    column_definitions = []
    if "id" not in structure:
        column_definitions.append("id INT AUTO_INCREMENT PRIMARY KEY")
    column_definitions.extend(
        [f'{enquote(key)} {structure[key]}' for key in structure.keys()])
...
def insert_into_mysql(mysql_cursor, collection_name, document):
    collection_name = camel_to_snake(collection_name)
    document = {camel_to_snake(key): value for key, value in document.items() if key not in [
        '_id', '_class'] or (key == '_id' and not SKIP_ID_FIELD) or (key == '_class' and not SKIP_CLASS_FIELD)}
    ...
    quoted_values_tuple = tuple(f"'{value}'" if isinstance(
        value, str) else value for value in values_tuple)
    ...
    while True:
        try:
            ...
            if 'Unknown column' in str(e):
                ...
                for field in missing_fields:
                    ...
                    mysql_cursor.execute(
                        f"ALTER TABLE {collection_name} ADD COLUMN {field} {field_type}")
            elif 'Incorrect datetime value' in str(e):
                field = re.search(r"column '([^']+)'", str(e)).group(1)
                value = re.search(r"'([^']+)'", str(e)).group(1)
                value_length = len(value)
                varchar_size = min(max(DEFAULT_VARCHAR_SIZE, (value_length //
                                   DEFAULT_VARCHAR_SIZE + 1) * DEFAULT_VARCHAR_SIZE), MAX_VARCHAR_LENGTH)
                mysql_cursor.execute(
                    f"ALTER TABLE {collection_name} MODIFY {field} VARCHAR({varchar_size})")
            else:
                raise
            ...
            if 'Data too long' in str(e):
                ...
                mysql_cursor.execute(
                    f"SELECT DATA_TYPE, CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '{collection_name}' AND COLUMN_NAME = '{field}'")
                ...
                if current_type == 'varchar':
                    ...
                    raise ValueError(
                        f"Cannot increase size of field {field} of type {current_type}")
                mysql_cursor.execute(
                    f"ALTER TABLE {collection_name} MODIFY {field} {new_type}")
            elif 'Data truncated' in str(e) or 'Incorrect integer value' in str(e):
                field = re.search(r"column '([^']+)'", str(e)).group(1)
                mysql_cursor.execute(
                    f"ALTER TABLE {collection_name} MODIFY {field} VARCHAR({DEFAULT_VARCHAR_SIZE})")
            else:
                raise


def main():
    st.title("MongoDB to MySQL Migration")

    # MongoDB connection input fields
    mongo_host = st.text_input("MongoDB Host", value="localhost")
    mongo_port = st.number_input(
        "MongoDB Port", value=27017, min_value=1, max_value=65535)
    mongo_db_name = st.text_input("MongoDB Database Name")

    # MySQL connection input fields
    mysql_host = st.text_input("MySQL Host", value="localhost")
    mysql_port = st.number_input(
        "MySQL Port", value=3306, min_value=1, max_value=65535)
    mysql_user = st.text_input("MySQL User")
    # mysql_password = st.text_input("MySQL Password", type="password")

    if st.button("Migrate Data"):
        # Perform migration here using the provided inputs
        try:
            # Connect to MongoDB
            with pymongo.MongoClient(host=mongo_host, port=mongo_port) as mongo_client:
                mongo_db = mongo_client[mongo_db_name]

                # Connect to MySQL
                with pymysql.connect(host=mysql_host, port=mysql_port, user=mysql_user) as mysql_conn:
                    mysql_cursor = mysql_conn.cursor()
                    local_mysql_db_name = mongo_db_name.replace('-', '_')
                    mysql_cursor.execute(
                        f"DROP DATABASE IF EXISTS {local_mysql_db_name}")
                    mysql_cursor.execute(
                        f"CREATE DATABASE {local_mysql_db_name}")
                    mysql_cursor.execute(f"USE {local_mysql_db_name}")

                    # Iterate over all collections in MongoDB
                    for collection_name in mongo_db.list_collection_names():
                        st.write(
                            f"\nMigrating data from MongoDB collection '{collection_name}' to MySQL...")
                        ...
                        # Create MySQL table
                        create_mysql_table(
                            mysql_cursor, collection_name, document)
